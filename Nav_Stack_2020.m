...
classdef Nav_Stack_2020
    ...
    % Controller parameters
        ...
        kin_com %--> Kinematic control commands
    ...
    methods
        function obj = Nav_Stack_2020(model, Th_lim, n, lm) %---> Constructor
            ...
            obj.N_hor = N;
            Q = zeros(3,3); Q(1,1) = 1; Q(2,2) = 10; Q(3,3) = 0.1; % Weighing matrices (states)
            R = zeros(2,2); R(1,1) = 0.5; R(2,2) = 0.05; % Weighing matrices (controls)
            obj.kin_com = [0, 0]; %--> Initial kinematic control commands
            
            % Trajectory tracking solver setup
            % Add optimization library path "Casadi"
            addpath(pwd + "\Dependencies\Casadi")
            import casadi.*
            x = SX.sym('x'); y = SX.sym('y'); theta = SX.sym('theta'); %--> Kinematic states
            states = [x; y; theta]; n_states = length(states); %--> Symbolic state vector
            v = SX.sym('v'); omega = SX.sym('omega'); %--> Kinematic controls
            controls = [v; omega]; n_controls = length(controls); %--> Symbolic controls vector
            rhs = [v * cos(theta); v * sin(theta); omega]; %--> State space model
            f = Function('f', {states, controls}, {rhs}); %--> Nonlinear mapping function f(x,u)
            U = SX.sym('U', n_controls, N); %--> Decision variables (sequence of control vectors leading to desired state)
            P = SX.sym('P', n_states + n_states); %--> Parameters vector "initial and reference states"
            X = SX.sym('X', n_states, (N + 1)); %--> States over the optimization problem / state space
            PHI = 0; %--> Objective function "add each time step loss to a summation vector PHI"
            g = [];  %--> State constraints vector
            st  = X(:, 1); %--> Initial state of the state trajectory
            g = [g; st - P(1 : n_states)]; %--> Initial condition constraint "Initial state must match Initial state of parameters vector"
            for k = 1 : N
                st = X(:, k);  con = U(:, k); %--> Current state and controls
                PHI = PHI + (st(1:3) - P(n_states + 1 : n_states + 3))' * Q * ...
                    (st(1:3) - P(n_states + 1 : n_states + 3)) + ...
                    con' * R * con; %--> Add current step loss to objective function
                st_next = X(:, k+1); 
                f_value = f(st, con); %--> Compute rate of change of state using state space model
                st_next_euler = st + (obj.dT_con * f_value); %--> Compute next state using euler formula
                g = [g; st_next - st_next_euler]; % Compute constraints on upcoming future states "must match state space evolution"
            end
            OPT_variables = [reshape(X, n_states * (N+1), 1); reshape(U, n_controls * N, 1)]; %--> Optimization variables
            nlp_prob = struct('f', PHI, 'x', OPT_variables, 'g', g, 'p', P); %--> Non-linear problem parameters
            opts = struct;
            ...
            obj.solver = nlpsol('solver', 'ipopt', nlp_prob, opts); %--> Create solver
        end
        ...
        function kincon = getkincon(obj) %--> Get current kinematic command
            kincon = obj.kin_com; 
        end
        function obj = Control(obj, Rt, Setp, zread, rpyread, lmread)
            if(round(Rt - obj.Tr, 1) >= obj.dT_con)
                ...
                n_states = 3; n_controls = 2;
                args = struct;
                args.lbg(1 : n_states * (obj.N_hor + 1)) = 0; %--> Equality constraints
                args.ubg(1 : n_states * (obj.N_hor + 1)) = 0; %--> Equality constraints
                args.lbx(1 : n_states * (obj.N_hor + 1), 1) = -inf; %--> States lower bound "no bound"
                args.ubx(1 : n_states * (obj.N_hor + 1), 1) = inf; %--> States upper bound "no bound"
                args.lbx(n_states * (obj.N_hor + 1) + 1 : 2 : n_states * (obj.N_hor + 1) + ...
                    n_controls * obj.N_hor, 1) = -sqrt((0.85 * obj.th_lim * 4 * cos(obj.tht) / obj.Dx)); 
                  %--> Surge velocity lower bound
                args.ubx(n_states * (obj.N_hor + 1) + 1 : 2 : n_states * (obj.N_hor + 1) + ...
                    n_controls * obj.N_hor, 1) = sqrt((0.85 * obj.th_lim * 4 * cos(obj.tht) / obj.Dx)); 
                  %--> Surge velocity upper bound
                args.lbx(n_states * (obj.N_hor + 1) + 2 : 2 : n_states * (obj.N_hor + 1) + ...
                    n_controls * obj.N_hor, 1) = -sqrt((0.15 * obj.th_lim * 4 * obj.rh / obj.Dth)); 
                  %--> Yaw velocity lower bound
                args.ubx(n_states * (obj.N_hor + 1) + 2 : 2 : n_states * (obj.N_hor + 1) + ...
                    n_controls * obj.N_hor, 1) = sqrt((0.15 * obj.th_lim * 4 * obj.rh / obj.Dth)); 
                  %--> Yaw velocity upper bound
                x0 = [obj.u(5) ; obj.u(6) ; obj.u(8)]; %--> Initial state
                xs = [Setp(1); Setp(2); Setp(4)]; %--> Reference / desired posture
                u0 = zeros(obj.N_hor, n_controls); %--> Control inputs along state space trajectory
                X0 = repmat(x0, 1, obj.N_hor + 1)'; %--> Initialization of the decision variables
                args.p = [x0; xs]; %--> Set the values of the parameters vector
                args.x0  = [reshape(X0', n_states * (obj.N_hor + 1), 1);
                    reshape(u0', n_controls * obj.N_hor, 1)];
                sol = obj.solver('x0', args.x0, 'lbx', args.lbx, 'ubx', args.ubx,...
                    'lbg', args.lbg, 'ubg', args.ubg,'p',args.p); %--> Get solution
                opt_con = [full(sol.x(n_states * (obj.N_hor + 1) + 1)), full(sol.x(n_states * 2))];
                  %--> Get controls only from the solution
                vcon = opt_con(1,1); %--> Surge velocity control command
                thcon = opt_con(1,2); %--> Yaw angular control command
                
                % Kinetic control
                kp = 500;
                ki = (obj.Dx * vcon + (kp / 2))^2 / obj.Mx; %--> Compute integral gain for velocity I controller
                taw_surge = (vcon - obj.u(1)) * (kp + (obj.dT_con * ki) / 2) + (obj.kin_com(1, 1) - u_k1(1)) * ...
                    (-kp + (obj.dT_con * ki) / 2) + ...
                    (sum(obj.pwm_th(2, 1 : 4)) * cos(obj.tht)) / (4 * cos(obj.tht)); %--> Surge portion of thrust
                if(taw_surge > 0.85 * obj.th_lim)
                    taw_surge = 0.85 * obj.th_lim; %--> Surge portion upper limit
                elseif(taw_surge < -0.85 * obj.th_lim)
                    taw_surge = -0.85 * obj.th_lim; %--> Surge portion lower limit
                end
                kp = 500; kd = 2 * sqrt(kp * obj.Mth); %--> Compute proportional and derivative gains for yaw PD
                taw_yaw = (kp * (thcon - obj.u(8) + obj.kin_com(2) - u_k1(8)) + 2 * kd * ...
                    (thcon - obj.u(8) - obj.kin_com(2) + u_k1(8)) - ...
                    (-obj.pwm_th(2,1) + obj.pwm_th(2,2) + obj.pwm_th(2,3) - obj.pwm_th(2,4)) * 4 * obj.rh) / ...
                    (4 * obj.rh); %--> Yaw portion of thrust     
                if(taw_yaw > 0.15 * obj.th_lim)
                    taw_yaw = 0.15 * obj.th_lim; %--> Yaw portion upper limit
                elseif(taw_yaw < -0.15 * obj.th_lim)
                    taw_yaw = -0.15 * obj.th_lim; %--> Yaw portion lower limit
                end
                obj.kin_com = [vcon, thcon]; %--> Update kinematic controls record
                Th_hor = [taw_surge - taw_yaw, taw_surge + taw_yaw, taw_surge + taw_yaw, ...
                    taw_surge - taw_yaw]; %--> Horizontal thruster command
